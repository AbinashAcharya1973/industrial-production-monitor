...
def clean_line(raw_line: str) -> str:
    ...
    then locate the first 10-field all-numeric CSV segment within what remains.
    ...
    # 3. Slide a 10-field window across the comma-split parts
    parts = cleaned.split(",")
    if len(parts) >= 10:
        for start in range(len(parts) - 9):
            fields = parts[start:start + 10]
            if all(f.strip().lstrip("-").isdigit() for f in fields):
