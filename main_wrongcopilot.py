...
def clean_line(raw_line: str) -> str:
    ...
    then locate the first 11-field all-numeric CSV segment within what remains.
    ...
    # 3. Slide an 11-field window across the comma-split parts
    #    Fields: PanelID,YY,MM,DD,HH,MM,SS,ZoneID,ConveyorID,Count,ProductSlno
    parts = cleaned.split(",")
    if len(parts) >= 11:
        for start in range(len(parts) - 10):
            fields = parts[start:start + 11]
...
def parse_packet(raw_line: str) -> dict | None:
    ...
    Format: PanelID,YY,MM,DD,HH,MM,SS,ZoneID,ConveyorID,Count,ProductSlno
    Example: 1,21,5,23,16,21,17,1,1,13,A001
    ...
    try:
        ...
        count        = int(parts[9])
        productslno  = parts[10]               # VARCHAR — keep as-is
...
class DatabaseManager:
    ...
    def _create_tables(self):
        with self._lock:
            ...
            """)
            # Migration: add productslno column to existing DBs that lack it
            existing = {
                row[1] for row in
    ...
    def query_production_table(
        ...
        (panel_id, year, month, day, zone, conveyor) — one row per conveyor line per day.

        productslno is NOT in GROUP BY to avoid duplicate rows when some records
        arrive without a slno. Instead we use MAX(p.productslno) to get the most
        recent non-null value per group, then LEFT JOIN products on that.
        ...
        SQL produced:
            ...
            FROM (
                SELECT
                    ...
                    MAX(productslno) AS productslno,
        ...
        sql = f"""
            SELECT
                agg.panel_id, agg.year, agg.month, agg.day,
                agg.zone, agg.conveyor,
                -- pick most-recent non-empty productslno for the group
                (SELECT p2.productslno FROM production p2
                   WHERE p2.panel_id = agg.panel_id
                     AND p2.year = agg.year
                     AND p2.month = agg.month
                     AND p2.day = agg.day
                     AND p2.zone = agg.zone
                     AND p2.conveyor = agg.conveyor
                     AND trim(p2.productslno) <> ''
                   ORDER BY p2.created_at DESC LIMIT 1) AS productslno,
                pr.partno,
            ...
            LEFT JOIN products pr ON pr.slno = (
                SELECT p2.productslno FROM production p2
                   WHERE p2.panel_id = agg.panel_id
                     AND p2.year = agg.year
                     AND p2.month = agg.month
    ...
    def query_24h_view(
        ...
        (panel_id, zone, conveyor) — one row per conveyor line.

        MAX(productslno) picks the most recent slno; LEFT JOIN products for partno.
        """
        hour_pivots = ",\n".join(
            f"    SUM(CASE WHEN hour={h} THEN count ELSE 0 END) AS hour_{h}"
            for h in range(24)
        ...
        sql = f"""
            SELECT
                agg.panel_id, agg.zone, agg.conveyor,
                -- most-recent non-empty productslno for the period/group
                (SELECT p2.productslno FROM production p2
                   WHERE p2.panel_id = agg.panel_id
                     AND p2.zone = agg.zone
                     AND p2.conveyor = agg.conveyor
...
class IndustrialMonitor(QMainWindow):
    ...
    def _build_prod_tab(self):
        ...
        cols  = ["Date", "Panel", "Zone", "Line Name", "Product Slno", "Part No", "Target/hr"] + hours
        self.prod_table = self._make_table(cols)
        # Fixed widths
        for w, c in zip([100, 55, 55, 140, 90, 90, 70], range(7)):
    ...
    def _on_line_received(self, raw_line: str):
        ...
        self._log(
            ...
            f"count={fields['count']}  productslno={fields['productslno']}"
        )
