...
class DatabaseManager:
    ...
    def query_production_table(
        ...
        (panel_id, year, month, day, zone, conveyor, productslno).

        JOIN products to also bring in partno.

        SQL (simplified):
            SELECT
                p.panel_id, p.year, p.month, p.day,
                p.zone, p.conveyor, p.productslno,
                pr.partno,
                SUM(CASE WHEN p.hour=0  THEN p.count ELSE 0 END) AS hour_0,
                ...
                SUM(CASE WHEN p.hour=23 THEN p.count ELSE 0 END) AS hour_23,
                SUM(p.count) AS day_total
            FROM production p
            LEFT JOIN products pr ON pr.slno = p.productslno
            [WHERE ...]
            GROUP BY p.panel_id, p.year, p.month, p.day,
                     p.zone, p.conveyor, p.productslno
            ORDER BY p.year, p.month, p.day, p.panel_id, p.zone, p.conveyor
        """
        hour_pivots = ",\n".join(
            f"    SUM(CASE WHEN p.hour={h} THEN p.count ELSE 0 END) AS hour_{h}"
            for h in range(24)
        )
        where, params = self._build_where(
            quick_filter, panel_id, zone, conveyor, alias="p"
        )
        sql = f"""
            SELECT
                p.panel_id, p.year, p.month, p.day,
    ...
    def query_24h_view(
        ...
        (panel_id, zone, conveyor, productslno), with LEFT JOIN to products.
        """
        hour_pivots = ",\n".join(
            f"    SUM(CASE WHEN p.hour={h} THEN p.count ELSE 0 END) AS hour_{h}"
            for h in range(24)
        ...
        sql = f"""
            SELECT
                p.panel_id, p.zone, p.conveyor,
                p.productslno,
                pr.partno,
            {hour_pivots},
                SUM(p.count) AS period_total
            FROM production p
            LEFT JOIN products pr ON pr.slno = p.productslno
            {where}
            GROUP BY p.panel_id, p.zone, p.conveyor, p.productslno
            ORDER BY p.panel_id, p.zone, p.conveyor
        """
        with self._lock:
            return self._conn.execute(sql, params).fetchall()
