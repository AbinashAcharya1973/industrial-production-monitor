...
def parse_packet(raw_line: str) -> dict | None:
    ...
    Format: PanelID,YY,MM,DD,HH,MM,SS,ZoneID,ConveyorID,Count
    ...
    try:
        ...
        year        = int(parts[1]) + 2000   # two-digit year → full year
        month       = int(parts[2])
        day         = int(parts[3])
        hour        = int(parts[4])
        minute      = int(parts[5])
        ...
        conveyor    = str(int(parts[8]))      # stored as TEXT in DB
        count       = int(parts[9])
...
class DatabaseManager:
    ...
    Schema (matches the DBeaver screenshot exactly):
    ...
    def query_production_table(
        self,
        quick_filter: str = "all",   # "today" | "7days" | "30days" | "all"
        panel_id:  str | None = None,
        zone:      str | None = None,
        conveyor:  str | None = None,
    ) -> list[sqlite3.Row]:
        """
        SELECT aggregated hourly counts grouped by (panel_id, year, month, day, zone, conveyor).

        Returns one row per unique date+line combination, with SUM(count) per hour
        as hour_0 … hour_23.

        SQL produced (example with no filters):
            SELECT
                panel_id, year, month, day, zone, conveyor,
                SUM(CASE WHEN hour=0  THEN count ELSE 0 END) AS hour_0,
                ...
                SUM(CASE WHEN hour=23 THEN count ELSE 0 END) AS hour_23,
                SUM(count) AS day_total
            FROM production
            GROUP BY panel_id, year, month, day, zone, conveyor
            ORDER BY year, month, day, panel_id, zone, conveyor
        """
        hour_pivots = ",\n".join(
            f"    SUM(CASE WHEN hour={h} THEN count ELSE 0 END) AS hour_{h}"
            for h in range(24)
        )
        where, params = self._build_where(quick_filter, panel_id, zone, conveyor)
        sql = f"""
            SELECT
                panel_id, year, month, day, zone, conveyor,
            {hour_pivots},
    ...
    def query_24h_view(
        ...
        (panel_id, zone, conveyor).  Each row shows total count per hour
        ...
        sql = f"""
            ...
            ORDER BY panel_id, zone, conveyor
        """
        with self._lock:
            return self._conn.execute(sql, params).fetchall()

    # ── STEP 2d: SELECT for filter dropdown population ────────
    def query_distinct_values(self) -> dict:
        """
        Returns distinct panel_id, zone, conveyor, and date strings
        for populating the filter combo boxes.
        """
        with self._lock:
            ...
            dates = [r[0] for r in self._conn.execute("""
                SELECT DISTINCT
                    printf('%04d-%02d-%02d', year, month, day) AS date_str
                FROM production
                ORDER BY year, month, day
            """).fetchall()]
        return {"panels": panels, "zones": zones,
                "conveyors": conveyors, "dates": dates}

    # ── STEP 2e: SELECT for stat cards ────────────────────────
    def query_stats(self) -> dict:
        """
        Returns aggregate stats used by the top stat cards:
            total_count   – SUM of all counts in the table
            total_rows    – COUNT(*) of raw records
            line_count    – number of distinct (panel, zone, conveyor) lines
        """
        with self._lock:
    ...
    # ── Utility ───────────────────────────────────────────────
    def _build_where(self, quick_filter, panel_id, zone, conveyor) -> tuple[str, list]:
        """Build the WHERE clause and params list for filter queries."""
        clauses, params = [], []
        ...
        if quick_filter == "today":
            ...
            # Compare using a computed integer YYYYMMDD for simplicity
            clauses.append(
    ...
    def export_csv(self, filepath: str):
        """Write the full raw production table to a CSV file."""
        with self._lock:
        ...
        with open(filepath, "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(["ID", "panel_id", "year", "month", "day",
            ...
            try:
                if ser.in_waiting > 0:
                    ...
                    try:
                        ...
                        self.error_occurred.emit(f"Binary data: {raw.hex()}")
            except Exception as e:
                self.error_occurred.emit(str(e))
                break
            time.sleep(0.01)
...
class IndustrialMonitor(QMainWindow):
    ...
    def _build_stat_row(self):
        row = QHBoxLayout()
        row.setSpacing(10)
        self.card_total  = StatCard("Total Production", "0",       "#00ff88")
        self.card_lines  = StatCard("Conveyor Lines",   "0",       "#00d4ff")
        self.card_pkts   = StatCard("Packets Received", "0",       "#00d4ff")
        self.card_port   = StatCard("Current COM Port", "N/A",     "#00ff88")
        self.card_clock  = StatCard("Last Update",      "—",       "#00ff88")
        self.card_uptime = StatCard("System Uptime",    "00:00:00","#00ff88")
    ...
    def _build_prod_tab(self):
        ...
        cols  = ["Date", "Panel", "Zone", "Line Name", "Target/hr"] + hours
        self.prod_table = self._make_table(cols)
        # Fixed widths
        for w, c in zip([100, 55, 55, 140, 70], range(5)):
            self.prod_table.setColumnWidth(c, w)
        for c in range(5, 5 + 24):
    ...
    def _build_24h_tab(self):
        ...
        layout.setContentsMargins(0, 8, 0, 0)
        hours = [f"{h:02d}:00" for h in range(24)]
        cols  = ["Panel", "Zone", "Conveyor", "Line Name"] + hours + ["Total"]
    ...
    def _on_line_received(self, raw_line: str):
        ...
        self._log(f"RX: {raw_line}")

        # ── STEP 1: Parse fields ──────────────────────────────
        fields = parse_packet(raw_line)
        if fields is None:
            self._on_serial_error(f"Invalid packet: {raw_line}")
        ...
        self._log(
            ...
            f"zone={fields['zone']}  conveyor={fields['conveyor']}  "
    ...
    def _refresh_ui_from_db(self):
        ...
        for r_idx, row in enumerate(rows):
            ...
            line_name = f"Zone{row['zone']} Conveyor{row['conveyor']}"
            cells = [
                date_str,
                row["panel_id"],
                row["zone"],
                line_name,
                "100",                          # Target/hr (configurable later)
...
#bottomBar { color: #00ff88; font-size: 10px; padding: 2px 0; }
